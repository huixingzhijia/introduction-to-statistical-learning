...
with(Wage,lines(age.grid,preds$fit,lwd=2,col="blue"))

matlines(age.grid,band,lwd=1,col="blue")
...
lines(dis.grid,sp.fit,type="l",col="red")
...
threshold<-1e-05
while (e < m_i && errors[e - 1] - errors[e] > threshold){
  for (i in 1:p){
...
#It seems like 10 interation can get us to the threshold,

plot(1:11,errors[3:13],type = "l")
which.min(errors)
points(11,errors[13],col="red")
